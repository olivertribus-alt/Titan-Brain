...
from core.trace import TraceRegistry
...
from core.types.message import Broadcast, CognitiveMessage
from core.types.payload import BasePayload
from core.types.priority import Priority
from core.types.telemetry import (
    CognitiveTelemetry,
    RuntimeTelemetry,
    Telemetry,
)
...
def test_trace_registry_invalid_max_records() -> None:
    with pytest.raises(
        ValueError, match="max_records must be greater than or equal to one"
    ):
...
def test_trace_registry_records_dispatch() -> None:
    ...
    message: CognitiveMessage[BasePayload] = CognitiveMessage(
        message_id=msg_id,
        trace_id=trace_id,
    ...
    report = DeliveryReport(
        message_id=msg_id,
        delivered_to=(create_module_id("Receiver"),),
        failures=(),
    )
...
def test_trace_registry_fifo_eviction() -> None:
    ...
    def _make_msg_and_report() -> tuple[CognitiveMessage[BasePayload], DeliveryReport]:
        ...
        msg: CognitiveMessage[BasePayload] = CognitiveMessage(
            message_id=mid,
            trace_id=create_trace_id(),
            source=create_module_id("Source"),
            destination=Broadcast(),
