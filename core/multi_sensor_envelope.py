...
Guarantees O(1) worst-case safety envelope union across heterogenous sensors.
"""
...
class MultiSensorEnvelopeEvaluator:
    ...
    def evaluate_fusion(
        ...
        if critical_stale_or_missing:
            reason_str = ", ".join(critical_stale_or_missing)
            return MultiSensorFusionResult(
                ...
                is_emergency=True,
                emergency_reason=f"CRITICAL_SENSOR_STALE: {reason_str}",
                timestamp_s=now,
            )
